...
class PyFsrsBasicSuite extends AnyFunSuite {
  ...
  test("repeated correct reviews clamp difficulty to 1.0") {
    val scheduler = Scheduler(Parameters())
    var card = Card()
  ...
  test("one card multiple schedulers") {
    val s2Learning = Scheduler(Parameters(learningSteps = List(1.minute, 10.minutes)))
    val s1Learning = Scheduler(Parameters(learningSteps = List(1.minute)))
    val s0Learning = Scheduler(Parameters(learningSteps = List.empty))
    val s2Relearning = Scheduler(Parameters(relearningSteps = List(1.minute, 10.minutes)))
    val s1Relearning = Scheduler(Parameters(relearningSteps = List(1.minute)))
    val s0Relearning = Scheduler(Parameters(relearningSteps = List.empty))
  ...
  test("parameter validation - one too high") {
    ...
    assertThrows[IllegalArgumentException] {
      Scheduler(Parameters(weights = badWeights))
    }
  }
  ...
  test("parameter validation - empty") {
    assertThrows[IllegalArgumentException] {
      Scheduler(Parameters(weights = List.empty))
    }
  }

  test("parameter validation - too few") {
    assertThrows[IllegalArgumentException] {
      Scheduler(Parameters(weights = Parameters.defaultWeights.dropRight(1)))
    }
  }

  test("parameter validation - too many") {
    assertThrows[IllegalArgumentException] {
      Scheduler(Parameters(weights = Parameters.defaultWeights ++ List(1.0, 2.0, 3.0)))
    }
  }

  test("learning card rate hard one learning step") {
    val firstStep = 10.minutes
    val scheduler = Scheduler(Parameters(learningSteps = List(firstStep)))
    var card = Card()
  ...
  test("learning card rate hard second learning step") {
    ...
    val scheduler = Scheduler(Parameters(learningSteps = List(firstStep, secondStep)))
    var card = Card()
  ...
  test("fuzz not applied to short intervals") {
    val scheduler1 = Scheduler(Parameters(withFuzzing = false))
